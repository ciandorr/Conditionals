...
% The standard test for presupposition is to look at inferences which a sentence gives rise to both when asserted, when asked as a polar (yes-no) question, and when embedded under various operations including negation and various kinds of modals.  Applying the negation test to conditionals is hard since it's not clear that we have any idiomatic way to negate a conditional (intuitions about ‘It is not the case that if $P$, $Q$’ are likely to be contaminated by the fact that this is the sort of sentence that only philosophers and linguists would produce); but the other tests do apply and support the claimed presupposition of nonvacuity.  When asked a conditional question like ‘Where is John if he's ’
